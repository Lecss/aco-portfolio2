...
class Ant():
	...
	def choose_next_node(self, curr_node):
		...
		neighbor_ph_sum = 0
		for n in neighbors: 
			ph= self.graph.edge[curr_node][n]["ph"]
			neighbor_ph_sum += ph
			heuristics = self.get_heuristics(n)
	...
	def get_heuristics(self,node):
		result = 0
		h1 = self.related_investments_heuristic(node)
		h2 = self.drug_ratio_heuristic(node)
		#print str(type(h1)) + ':' + str(h1) + ":" + str(node)
		result+= h1 + h2
	...
	def related_investments_heuristic(self, node):
		...
		drug = self.graph.node[node]["drug"]
		stage = self.graph.node[node]["stage"]
		total_stages = len(self.graph.graph['drug_stages'][drug.name])
		
		if "One" in stage.name:
			return 0.4
		elif "Two" in stage.name:
			return 0.6
		elif "Three" in stage.name:
			return 1
	...
	def update_pheromones(self, evap_rate):
		...
		for i in range(0, len(self.path)-1): 
			...
			if self.graph.edge[from_stage][to_stage]["ph"] < 0: 
				self.graph.edge[from_stage][to_stage]["ph"] = 0
